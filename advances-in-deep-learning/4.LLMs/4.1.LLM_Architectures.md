...
#### 2.4.6 Byte-Level BPE (BBPE)
- **Overview**: BBPE is the variant of BPE used by virtually all modern LLMs (GPT-2 onwards, Llama, Mistral, Qwen). Instead of running merges over *Unicode characters*, it runs them over the *raw UTF-8 bytes* of the text. The base vocabulary is therefore always exactly 256 symbols (byte values 0–255), regardless of language.
- **Motivation**: Character-level BPE has to decide up front which Unicode characters are "known". Anything outside that set becomes an out-of-vocabulary token (`<UNK>`), which is lossy and hurts multilingual text, emojis, and code. Unicode also has ~150,000 code points, so a character-level base vocabulary is either huge or incomplete. Bytes solve both problems: 256 symbols cover *every* possible string.
- **Process**:
  1. Encode the text as UTF-8, giving a sequence of bytes. Each byte is an initial token, so the starting vocabulary is fixed at 256.
  2. Run the standard BPE merge loop (count the most frequent adjacent pair, merge it, repeat) over byte sequences until the target vocabulary size $n$ is reached.
  3. Final vocabulary size = 256 base bytes + number of learned merges + special tokens.
- **Example**: The word "hello" is the byte sequence `[104, 101, 108, 108, 111]`. A CJK character such as 世 is 3 UTF-8 bytes `[0xE4, 0xB8, 0x96]`; if 世 is frequent in the corpus, BPE merges those 3 bytes into one token, otherwise it stays as multiple byte tokens. This is exactly the behaviour observed in §2.5, where some Chinese characters are one token and others are split.
- **Key Properties**:
  - **No `<UNK>` token ever**: any byte string is representable, so the tokenizer is *lossless* — `decode(encode(s)) == s` for all $s$.
  - **Language-agnostic**: no per-language vocabulary engineering; the same tokenizer handles English, Chinese, emojis, base64 blobs, and binary-ish data.
  - **Byte fallback**: unseen or rare characters degrade gracefully into their constituent byte tokens rather than being dropped.
- **Byte-Level Pre-tokenization Trick (GPT-2)**: BPE merges are not allowed to cross pre-token boundaries (words, numbers, punctuation runs). GPT-2 additionally maps the 256 byte values into a set of 256 *printable* Unicode characters (e.g. byte `0x20` for space → `Ġ`, newline → `Ċ`) so the vocabulary file stays human-readable and whitespace-safe. This is why token strings in tokenizer dumps look like `Ġthe`, `Ġcat`, `ĊĊ`.
- **Trade-offs vs. character-level BPE**:
  - **Cost**: non-Latin scripts consume more tokens for the same amount of text, since one character can be 2–4 bytes and may not be fully merged. This is the main reason non-English prompts are more expensive and effectively have a shorter context.
  - **Sequence length**: for a fixed vocabulary size, BBPE compresses slightly worse than character-level BPE, because part of the vocabulary budget is spent re-learning multi-byte characters that a character-level scheme gets for free.
  - **Inherited BPE issues**: BBPE does not fix the problems in §2.4.5 (non-unique tokenizations, poor character-level counting, digit-grouping in arithmetic, glitch tokens). Models mitigate the math case separately, e.g. by forcing single-digit or 3-digit-grouped number tokenization.
- **Partial-token decoding**: because a token can end mid-character, streaming output must buffer bytes until they form a valid UTF-8 sequence. Practical implementations emit a replacement character (`�`) or hold the bytes back until the next token arrives.
- **Variants**: SentencePiece (Llama, T5, Gemma) implements the same idea via a `byte_fallback` option — it uses a Unicode-level unigram/BPE vocabulary but represents any unmatched character as `<0xNN>` byte tokens, giving BBPE's losslessness with a character-oriented vocabulary.

#### 2.4.7 Other Tokenization Methods
